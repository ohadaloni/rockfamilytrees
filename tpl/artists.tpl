<table border="0">
	...
	{foreach from=$artists item=artist}
		<tr class="rftRow{if $artist.id == $currentArtist} currentArtist{/if}">
			<td>
				<a {* {if $band}{/if} *} href="?action=artist&amp;artistId={$artist.id}">{$artist.name|htmlspecialchars}</a>
	...
	{if $band && $smarty.session.rftId}
		<tr class="rftHeaderRow">
			<td>
				<form method="post" id="newArtistForm">
					<input type="text" size="30" name="artistName" />
					<input type="hidden" name="action" value="addArtistToBand" />
