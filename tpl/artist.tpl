...
<table border="0">
	<tr class="rftFormRow">
		<td colspan="2">
			<a href="?action=artist&amp;artistId={$artist.id}"><img border="0" width="16" height="16" src="images/refresh.png" alt="Reload" title="Reload" /></a>
			...
			{if $isFavorite}
				<a href="?action=removeFavoriteArtist&artistId={$artist.id}"><img width="16" height="16" border="0" src="images/removeFavorite.png"
	...
	{if $artist.createdBy == $user.id || $user.status == "Admin" || $user.status == "superAdmin" || $user.numOps > $adminNumOps}
		<tr class="rftFormRow">
			<td colspan="2">
					<form method="post" id="changeArtistForm">
						<input type="text" size="30" name="artistName" value="{$artist.name|htmlspecialchars}" />
						<input type="hidden" name="artistId" value="{$artist.id}" />
						<input type="hidden" name="action" value="changeArtist" />
	...
	<tr class="rftFormRow">
		...
		<td>
			...
			{if $artist.createdBy != $smarty.session.rftId}
				<a href="?action=userHome&amp;userId={$artist.createdBy}"><img width="16" height="16" border="0" src="images/home.png"
