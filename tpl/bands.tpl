<table border="0">
	<tr class="rftHeaderRow">
		<td colspan="3">
			...
			{if $user && ! $artist}
				...
				<a href="?action=unFavoriteAll"><img width="16" height="16" border="0" src="images/delete.png"
	...
	{foreach from=$bands item=band}
		<tr class="rftRow{if $band.id == $currentBand} currentBand{/if}">
			<td>
				<a {* {if $artist}{/if} *}href="?action=band&amp;bandId={$band.id}">{$band.name|htmlspecialchars}</a>
		...
		<tr class="rftHeaderRow">
			<td>
				<form method="post" id="newArtistForm">
					...
					<input type="hidden" name="action" value="addBandToArtist" />
